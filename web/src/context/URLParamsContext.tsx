...
interface IURLParamsProviderProps {
  ...
  filterHandlers: {
    handleToggleBooleanField: (e: React.ChangeEvent<HTMLInputElement>) => void;
...
const URLParamsProvider: React.FC = ({ children }) => {
  ...
  const variables: BasicSearchVariables = useMemo(
    () => ({
      ...
      checkIn:
        (searchParams.get("check-in") as string) || new Date().toString(),
      checkOut:
        (searchParams.get("check-out") as string) || new Date().toString(),
  ...
  const handleToggleBooleanField = (e: React.ChangeEvent<HTMLInputElement>) => {
